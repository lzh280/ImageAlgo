...
void MainWindow::on_pushButton_openImg_clicked()
{
    ...
    QPixmap overView = QPixmap::fromImage(sourceImg);
    if(overView.width() > overView.height())
        overView = overView.scaledToWidth(280);
    else
        overView = overView.scaledToHeight(280);
    ui->label_imgOverview->setGeometry(ui->label_imgOverview->x(),ui->label_imgOverview->y(),
                                       overView.width(),overView.height());
    ui->label_imgOverview->setPixmap(overView);
...
void MainWindow::on_actionAbout_triggered()
{
    ...
    aboutBox->addButton(tr("I Know"), QMessageBox::AcceptRole);
    aboutBox->exec();
}
...
void MainWindow::initDock()
{
    ...
    QPixmap spacer(280,280);
    spacer.fill(Qt::white);
    ui->label_imgOverview->setPixmap(spacer);
    ...
    connect(LB_DebugHandle::Instance(),&LB_DebugHandle::debugInfo,this,[=](const QString &str, const QColor &color, bool popup) {
        ...
        if(popup) {
            ...
            aboutBox->addButton(tr("OK"), QMessageBox::AcceptRole);
            aboutBox->exec();
        }
    });
